...
void test_bundle_parse__v2(void)
{
	...
	cl_git_pass(git_bundle__read_header(&header, "bundle/v2.header"));
...
void test_bundle_parse__v3(void)
{
	...
	cl_git_pass(git_bundle__read_header(&header, "bundle/v3.header"));
...
void test_bundle_parse__v3_sha256(void)
{
	...
	cl_git_pass(
	        git_bundle__read_header(&header, "bundle/v3_sha256.header"));
	cl_assert(header.version == 3);
...
void test_bundle_parse__invalid(void)
{
	...
	cl_git_fail_with(
	        GIT_EINVALID,
	        git_bundle__read_header(&header, "bundle/invalid.header"));
	git_bundle_header_dispose(&header);
}
...
void test_bundle_parse__bundle_does_not_exist(void)
{
	...
	cl_git_fail_with(
	        GIT_ENOTFOUND,
	        git_bundle__read_header(
	                &header, "bundle/does_not_exist.header"));
	git_bundle_header_dispose(&header);
}
...
void test_bundle_parse__bad_oid(void)
{
	...
	cl_git_fail_with(
	        GIT_ERROR,
	        git_bundle__read_header(&header, "bundle/bad_oid.header"));
	git_bundle_header_dispose(&header);
}
